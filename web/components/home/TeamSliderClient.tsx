 ...
 *
 * Data is fetched by the async server wrapper in TeamSlider.tsx and passed in
 * as a prop, so admin-dashboard changes are reflected without a redeploy.
 *
 * The wrapper lives in its own file rather than at the foot of this one: a
 * "use client" directive marks the entire module as client, so a `getTeam`
 * import sitting beside it would drag lib/cms.ts — and its `server-only`
 * guard — into the browser bundle and fail the build.
 */
